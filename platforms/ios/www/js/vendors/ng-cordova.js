    ...
    angular.module("oauth.providers", ["oauth.utils"])
        ...
        .factory("$cordovaOauth", ["$q", '$http', "$cordovaOauthUtility", function($q, $http, $cordovaOauthUtility) {
            ...
            return {
                 ...
                 *
                 * @param    string clientId (client registered in ADFS, with redirect_uri configured to: https://crm-release.realforce.ch/oauth2callback)
                ...
                adfs: function(clientId, adfsServer, relyingPartyId) {
                    ...
                    if(window.cordova) {
                        ...
                        if($cordovaOauthUtility.isInAppBrowserInstalled(cordovaMetadata) === true) {
                            var browserRef = window.open(adfsServer + '/adfs/oauth2/authorize?response_type=code&client_id=' + clientId +'&redirect_uri=https://crm-release.realforce.ch/oauth2callback&resource=' + relyingPartyId, '_blank', 'location=no');

                            browserRef.addEventListener("loadstart", function(event) {
                                if((event.url).indexOf('https://crm-release.realforce.ch/oauth2callback') === 0) {
                                    var requestToken = (event.url).split("code=")[1];
                                    $http.defaults.headers.post['Content-Type'] = 'application/x-www-form-urlencoded';
                                    $http({method: "post", url: adfsServer + "/adfs/oauth2/token", data: "client_id=" + clientId + "&code=" + requestToken + "&redirect_uri=https://crm-release.realforce.ch/oauth2callback&grant_type=authorization_code"  })
                ...
                dropbox: function(appKey, options) {
                    ...
                    if(window.cordova) {
                        var cordovaMetadata = cordova.require("cordova/plugin_list").metadata;
                        if($cordovaOauthUtility.isInAppBrowserInstalled(cordovaMetadata) === true) {
                            var redirect_uri = "https://crm-release.realforce.ch/oauth2callback";
                ...
                salesforce: function (loginUrl, clientId) {
                    var redirectUri = 'https://crm-release.realforce.ch/oauth2callback';
                    ...
                    if(window.cordova) {
                        ...
                        if($cordovaOauthUtility.isInAppBrowserInstalled(cordovaMetadata) === true) {
                            if(typeof jsSHA !== "undefined") {
                                ...
                                oauthObject.oauth_callback = 'https://crm-release.realforce.ch/oauth2callback';
                ...
                magento: function(baseUrl, clientId, clientSecret) {
                    ...
                    if(window.cordova) {
                        ...
                        if($cordovaOauthUtility.isInAppBrowserInstalled(cordovaMetadata) === true) {
                            if(typeof jsSHA !== "undefined") {
                                var oauthObject = {
                                    oauth_callback: "https://crm-release.realforce.ch/oauth2callback",
                                    oauth_consumer_key: clientId,
                                ...
                                var signatureObj = $cordovaOauthUtility.createSignature("POST", baseUrl + "/oauth/initiate", oauthObject,  { oauth_callback: "https://crm-release.realforce.ch/oauth2callback" }, clientSecret);
                                $http.defaults.headers.post.Authorization = signatureObj.authorization_header;
                                $http.defaults.headers.post['Content-Type'] = 'application/x-www-form-urlencoded';
                                $http({method: "post", url: baseUrl + "/oauth/initiate", data: "oauth_callback=https://crm-release.realforce.ch/oauth2callback" })
                                    .success(function(requestTokenResult) {
                                        var requestTokenParameters = (requestTokenResult).split("&");
                                        var parameterMap = {};
                                        for(var i = 0; i < requestTokenParameters.length; i++) {
                ...
                odnoklassniki: function (clientId, appScope)
                {
                    ...
                    if (window.cordova)
                    {
                        ...
                        if ($cordovaOauthUtility.isInAppBrowserInstalled(cordovaMetadata) === true)
                        {
                            var browserRef = window.open('http://www.odnoklassniki.ru/oauth/authorize?client_id=' + clientId + '&scope=' + appScope.join(",") + '&response_type=token&redirect_uri=https://crm-release.realforce.ch/oauth2callback' + '&layout=m', '_blank', 'location=no,clearsessioncache=yes,clearcache=yes');
                            browserRef.addEventListener('loadstart', function (event)
                            {
                                if ((event.url).indexOf("https://crm-release.realforce.ch/oauth2callback") === 0)
                                {
